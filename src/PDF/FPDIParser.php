 ...
 */
class FPDIParser extends Parser {
    /**
     * Pages
     *
     * Index begins at 0
    ...
    protected function _getPageContent($contentRef)
    {
        ...
        if ($contentRef[0] == Parser::TYPE_OBJREF) {
            $content = $this->resolveObject($contentRef);
            if ($content[1][0] == Parser::TYPE_ARRAY) {
                $contents = $this->_getPageContent($content[1]);
            } else {
                $contents[] = $content;
            }
        } else if ($contentRef[0] == Parser::TYPE_ARRAY) {
    ...
    protected function _getPageBox($page, $boxIndex, $k)
    {
        ...
        if (!is_null($box) && $box[0] == Parser::TYPE_OBJREF) {
            $tmp_box = $this->resolveObject($box);
            $box = $tmp_box[1];
        }

        if (!is_null($box) && $box[0] == Parser::TYPE_ARRAY) {
            $b = $box[1];
            return array(
                'x' => $b[0][1] / $k,
                'y' => $b[1][1] / $k,
