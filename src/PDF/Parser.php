...
use fpdi\Filter\ASCII85;
use fpdi\Filter\ASCIIHexDecode;
use fpdi\Filter\LZW;
...
class Parser {
    ...
    protected function _unFilterStream($obj)
    {
        ...
        if (isset($obj[1][1]['/Filter'])) {
            ...
            if ($filter[0] == Parser::TYPE_OBJREF) {
                $tmpFilter = $this->resolveObject($filter);
                $filter = $tmpFilter[1];
            }

            if ($filter[0] == Parser::TYPE_TOKEN) {
                $filters[] = $filter;
            } else if ($filter[0] == Parser::TYPE_ARRAY) {
                $filters = $filter[1];
            }
        }
